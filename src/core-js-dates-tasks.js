...
function getNextFriday(date) {
  ...
  newDate.setUTCDate(
    date.getUTCDate() + (dayOfWeek < 5 ? 5 - dayOfWeek : 12 - dayOfWeek)
  );
  return newDate;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (
    1 + new Date(new Date(dateEnd) - new Date(dateStart)).getTime() / 86_400_000
  );
}
...
function getWeekNumberByDate(date) {
  const dayOfYear =
    1 + Math.floor((date - new Date(date.getFullYear(), 0, 1)) / 86_400_000);
  const dayOfWeek = date.getDay() > 0 ? date.getDay() : 7;
  const week = Math.floor((10 + dayOfYear - dayOfWeek) / 7);
  return week;
}
...
function getNextFridayThe13th(date) {
  ...
  if (date.getDate() > 13) currentDate.setMonth(date.getMonth() + 1);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      result.push(currentDate.toLocaleDateString('fr').replaceAll('/', '-'));
